...
export class EventSubChannelRewardUpdateSubscription extends EventSubSubscription<EventSubChannelRewardEvent> {
	...
	protected async _subscribe(): Promise<HelixEventSubSubscription | undefined> {
		if (this._rewardId) {
			return this._client._config.managed
				? await this._client._config.apiClient.eventSub.subscribeToChannelRewardUpdateEventsForReward(
						this._userId,
						this._rewardId,
		...
		return this._client._config.managed
			? await this._client._config.apiClient.eventSub.subscribeToChannelRewardUpdateEvents(
					this._userId,
					await this._getTransportOptions(),
			  )
			: undefined;
